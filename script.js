...
let timerSeconds = 0;
let isPaused = false;
let examState = { answers: {}, marked: {}, startTime: null, elapsedTime: 0, completed: false };
let tickSound = new Audio('clock-ticking.mp3');
tickSound.loop = true;
...
function clearAllExamStates() {
    ...
    examState = { answers: {}, marked: {}, startTime: null, elapsedTime: 0, completed: false };
}
...
function navigateTo(stepName, pushState = true, resetExam = false) {
    ...
    if (stepName !== 'exam' && timerInterval) {
        clearInterval(timerInterval);
        timerInterval = null;
    }
...
function saveExamState() {
    ...
    if (session.mode === 'full' && timerSeconds > 0) {
        ...
        timerInterval = setInterval(() => {
            if (!isPaused) {
                ...
                if (timerSeconds <= 0) {
                    ...
                    if (document.body.classList.contains('body-red-pulse')) {
                        document.body.classList.remove('body-red-pulse');
                    }
                    processResults();
                }
            }
...
function getExamDuration() {
    ...
    const baseDurations = { 'urt': 90, 'toc': 90 };
...
function updateTimerDisplay() {
    ...
    if (timerSeconds <= 60 && timerSeconds > 0) {
        ...
        if (!document.body.classList.contains('body-red-pulse')) {
            document.body.classList.add('body-red-pulse');
        }

        if (!isTickPlaying && !isPaused) {
            tickSound.play().then(() => {
                isTickPlaying = true;
...
function renderExam() {
    ...
    if (session.mode === 'full') {
        timerContainer.style.display = isCompleted ? 'none' : 'block';
        questionNav.style.display = 'none';
        submitBtn.style.display = isCompleted ? 'none' : 'inline-block';
...
function renderFullExam() {
            ...
            <div class="question-card" id="q-card-${qCount}">
                ...
                <div class="options-container">
                    ${item.options.map((opt, idx) => `
                        <label class="option-label ${isCompleted && examState.answers[qCount] === idx ? (idx === item.correct ? 'correct' : 'incorrect') : ''} ${isCompleted && idx === item.correct ? 'correct-answer' : ''}">
                            <input type="radio" name="q${qCount}" value="${idx}" ${disabledAttr} onchange="saveExamState()"> ${opt}
                        </label>`).join("")}
                </div>
            </div>`;
            qCount++;
    ...
    html += `
        <div class="exam-end-message" style="text-align: center; font-weight: bold; margin-top: 40px; padding: 20px; font-size: 1.2rem; color: var(--text-secondary);">
            [ انتهت الأسئلة ]
        </div>
    `;
...
function renderQuestionByQuestion() {
    ...
    if (currentQuestionIndex === totalQuestions - 1) {
        ...
        endMsg.innerHTML = "[ انتهت الأسئلة ]";
        endMsg.style.cssText = "text-align: center; font-weight: bold; margin-top: 30px; padding: 10px; color: var(--text-secondary); font-size: 1.2rem;";
        document.getElementById("exam-container").appendChild(endMsg);
    }
...
function nextQuestion() {
    if (currentQuestionIndex < getQuestionList().length - 1) {
        ...
        const confirmFinish = confirm("Are you sure you want to finish the exam and see the results?");
        if (confirmFinish) {
            processResults();
        }
    }
